...
// Добавить счетчик числа тмц в локальное хранилище
if(!localStorage.getItem('goods')) {
  localStorage.setItem('goods', JSON.stringify([]))
}

// Сохранение новых рабочих
    ...
    if (result.isConfirmed) {
      ...
      for(let i=0; i<goods.length; i++) {
        if(goods[i][0] == e.target.dataset.delete) {
          goods.splice(i,1) // удалить запись
          localStorage.setItem('goods', JSON.stringify(goods))
          update_goods()
        }
      }
...
document.querySelector('.cart').addEventListener('input', function(e) {  
  ...
  for(let i=0; i<goods.length; i++) {
    if(goods[i][0] == e.target.dataset.goodid) {
      ...
      localStorage.setItem('goods', JSON.stringify(goods))
      update_goods()
      // Поставить фокус в поле и передвинуть курсор в конец
